...
- Add `impl From<hyper::Body> for reqwest_impersonate::Body`.
...
- Add conversion of `reqwest_impersonate::ClientBuilder` to `reqwest_impersonate::blocking::ClientBuilder`.
- Add `headers_mut()` to `reqwest_impersonate::blocking::Response`.
...
- Implement `TryFrom<http::Request>` for `Request`.
  ...
  - This is technically a breaking change, but was a mistake. It was not valid to convert from an `http::Request` to a `reqwest_impersonate::Request` in an infallible fashion. It would panic if the conversion was not possible. Instead, the implementation has been changed to `TryFrom` to indicate it could fail.

## v0.10.4

- Add `trust-dns` optional feature to change DNS resolver.
- Add `bytes()` method to `reqwest_impersonate::blocking::Response`.
- Add `buffer()` method to `reqwest_impersonate::blocking::Body`.
- Implement `From<http::Request>` for `reqwest_impersonate::Request`.
...
- Change default `Client` API to async. The previous blocking client API is available at `reqwest_impersonate::blocking`.
...
- Change default feature set to reduce unnecessary dependencies. Most features are disabled by default:
  - `blocking`: The `reqwest_impersonate::blocking` (synchronous) client API.
...
- Change Redirect API:
  - Renamed types to be part of the `redirect` module (for example, `reqwest_impersonate::RedirectPolicy` is now `reqwest_impersonate::redirect::Policy`).
...
- Add `reqwest_impersonate::async::multipart` support, similar to the synchronous API.
- Adds `default-tls-vendored` optional feature to vendor OpenSSL.

### Fixes

- Fix panic from top-level `reqwest_impersonate::get` if client builder fails to build.
- Removed timeout waiting for `reqwest_impersonate::Client` runtime to startup.
...
- Moved `request::unstable::async` to `reqwest_impersonate::async`.
...
- Changed `RequestBuilder` to be a by-value builder of by-ref.
  ...
  how to change it.
- Removed the `unstable` cargo-feature, and moved `reqwest_impersonate::unstable::async`
  to `reqwest_impersonate::async`.
...
- The upgrade to `hyper` 0.12 means a temporary removal of the typed headers.
  ...
  client
      .get("https://hyper.rs")
      .header_011(reqwest_impersonate::hyper_011::header::UserAgent::new("hallo"))
      .send()?;
  ```
...
- A default timeout for `reqwest_impersonate::Client` is used set to 30 seconds (#181)
...
- Upgrade hyper to 0.11
  ...
  - An `unstable` Cargo feature to enable `reqwest_impersonate::unstable::async`.
...
- `Error` is no longer an `enum`, but an opaque struct. Details about it can be checked with `std::error::Error::cause()`, and methods on `reqwest_impersonate::Error` include `is_http()`, `is_serialization()`, and `is_redirect()`.
...
- Add `reqwest_impersonate::mime` (https://github.com/seanmonstar/reqwest/commit/0615c6d65e03ba9cb5364169c9e74f4f2a91554b)
