...
async fn zstd_case(response_size: usize, chunk_size: usize) {
    ...
    // Use zstd crate for encoding
    let mut encoder = zstd::Encoder::new(Vec::new(), 0).unwrap();
    encoder.write_all(content.as_bytes()).unwrap();
    let zstd_content = encoder.finish().unwrap();
    ...
    let server = server::http(move |req| {
        ...
        let zstd = zstd_content.clone();
        async move {
            let len = zstd.len();
            let stream =
                futures_util::stream::unfold((zstd, 0usize), move |(zstd, pos)| async move {
                    let chunk = zstd.chunks(chunk_size).nth(pos)?.to_vec();

                    Some((chunk, (zstd, pos + 1)))
                });
