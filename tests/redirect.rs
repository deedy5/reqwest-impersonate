...
async fn test_redirect_301_and_302_and_303_changes_post_to_get() {
    ...
    for &code in codes.iter() {
        ...
        assert_eq!(
            res.headers()
                .get(reqwest_impersonate::header::SERVER)
                .unwrap(),
            &"test-dst"
        );
    }
...
fn test_redirect_307_does_not_try_if_reader_cannot_reset() {
    ...
    for &code in codes.iter() {
        ...
        let res = client
            .post(&url)
            .body(reqwest_impersonate::blocking::Body::new(&b"Hello"[..]))
            .send()
            .unwrap();
...
async fn test_redirect_removes_sensitive_headers() {
    ...
    reqwest_impersonate::Client::builder()
        ...
        .header(
            reqwest_impersonate::header::COOKIE,
            reqwest_impersonate::header::HeaderValue::from_static("foo=bar"),
        )
        .send()
        .await
        .unwrap();
...
async fn test_redirect_policy_can_return_errors() {
    ...
    let err = reqwest_impersonate::get(&url).await.unwrap_err();
...
async fn test_redirect_policy_can_stop_redirects_without_an_error() {
    ...
    assert_eq!(res.url().as_str(), url);
    assert_eq!(res.status(), reqwest_impersonate::StatusCode::FOUND);
}
...
async fn test_invalid_location_stops_redirect_gh484() {
    ...
    let res = reqwest_impersonate::get(&url).await.unwrap();
