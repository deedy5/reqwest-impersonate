...
async fn test_dns_cache_basic_functionality() {
    ...
    let result1 = client.get(&format!("http://{}/", domain)).send().await;
    ...
    let result2 = client.get(&format!("http://{}/", domain)).send().await;
...
async fn test_dns_cache_different_domains() {
    ...
    for domain in &domains {
        let result = client.get(&format!("http://{}/", domain)).send().await;
...
async fn test_dns_cache_error_handling() {
    ...
    let result = client
        .get("http://nonexistent.invalid.domain.test/")
        .send()
        .await;
